...
- `p`: `{"t":"p","x":12.5,"y":-3.2,"d":"d","i":"pointer"}` (`d` или `m`, `i` optional)
- `i`: `{"t":"i","i":"pointer_clicked"}` (icon update без изменения позиции)
- `bye`: `{"t":"bye"}`

Outbound:

- `b`: `{"t":"b","c":[[cid,sid,x,y,d,iconKey], ...]}`

`iconKey` валидируется по whitelist (`ALLOWED_ICON_KEYS`) и нормализуется к
`DEFAULT_ICON_KEY` при невалидном значении.

Legacy-форматы inbound сервером считаются невалидными.
...
- При inbound-сообщении `i` сервер выполняет **fast-path broadcast** сразу
  (без ожидания следующего тика `BROADCAST_INTERVAL_MS`) для заметной смены
  иконки на кликах.
